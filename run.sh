...
if [ ! -n "$WERCKER_SLACK_NOTIFY_FAILED_MESSAGE" ]; then
  if [ ! -n "$DEPLOY" ]; then
    WERCKER_SLACK_NOTIFY_FAILED_MESSAGE="$WERCKER_APPLICATION_OWNER_NAME/$WERCKER_APPLICATION_NAME: <$WERCKER_BUILD_URL|build> of $WERCKER_GIT_BRANCH by $WERCKER_STARTED_BY failed.
    @$WERCKER_STARTED_BY-- for breaking the build!
    herbie Y U NO  test your code $WERCKER_STARTED_BY!
    "
    export WERCKER_SLACK_NOTIFY_FAILED_MESSAGE
  else
